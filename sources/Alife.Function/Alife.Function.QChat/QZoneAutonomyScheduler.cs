...
public sealed class QZoneAutonomyScheduler
{
    ...
    public QZoneAutonomyDecision EvaluatePostCandidate(QZoneAutonomyContext context)
    {
        ...
        lock (syncRoot)
        {
            ...
            if (IsWithinPostWindow(now, context.Settings) == false)
            {
                if (ShouldDeferCandidate(state, now, context.Settings))
                {
                    state = DeferCandidate(
                        ...
                        now,
                        context.Settings,
                        IsStaleCandidate(state, state.NextPostCandidateAt, now));
                    SaveUnsafe(state);
                }
            ...
            if (IsStaleCandidate(state, nextCandidate, now))
            {
    ...
    bool ShouldDeferCandidate(QZoneAutonomyState state, DateTimeOffset now, QZoneAutonomySettings settings)
    {
        ...
        return IsStaleCandidate(state, nextCandidate, now)
            || IsWithinPostWindow(nextCandidate, settings) == false;
    }

    static bool IsStaleCandidate(
        QZoneAutonomyState state,
        DateTimeOffset? candidate,
        DateTimeOffset now) =>
        IsMissedPostTarget(state, now)
        || candidate is { } scheduledAt
        && DateOnly.FromDateTime(scheduledAt.DateTime) < DateOnly.FromDateTime(now.DateTime);
