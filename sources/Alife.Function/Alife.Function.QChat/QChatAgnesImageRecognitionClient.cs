...
public sealed class QChatAgnesImageRecognitionClient(
    HttpClient httpClient,
    Func<string?> apiKeyProvider,
    string? endpoint = null) : IQChatImageRecognitionClient
{
    const string DefaultEndpoint = "https://apihub.agnes-ai.com/v1/chat/completions";
    ...
    {
        string model = string.IsNullOrWhiteSpace(request.Model) ? "agnes-2.0-flash" : request.Model.Trim();
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(content))
            {
                return QChatImageRecognitionProviderResult.Fail(
                    ...
                    "missing_choices_message_content");
            }
    ...
    Uri ResolveEndpoint(string? requestEndpoint = null)
    {
        ...
        if (Uri.TryCreate(configured, UriKind.Absolute, out Uri? absolute))
            return absolute;
        return new Uri(configured, UriKind.Relative);
    }
    ...
    static string? ExtractContent(string body)
    {
        ...
        if (first.TryGetProperty("message", out JsonElement message) == false)
            return null;
        if (message.TryGetProperty("content", out JsonElement content) == false)
            return null;
    ...
    static QChatImageRecognitionTokenUsage? ExtractUsage(string body)
    {
        using JsonDocument document = JsonDocument.Parse(body);
        JsonElement root = document.RootElement;
        if (root.TryGetProperty("usage", out JsonElement usage) == false ||
    ...
    {
        if (element.TryGetProperty(propertyName, out JsonElement property) == false)
            return null;
