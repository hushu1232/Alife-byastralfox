    ...
    : InteractivePlugin<MemoryService>, IConfigurable<MemoryConfig>
{
    ...
    public async Task Recall(XmlExecutorContext ctx,
        [Description("存档索引（如：0-20240101120000-20240101130000）")] string index)
    {
    ...
    [XmlFunction]
    [Description("创建一个永久（最高层）记忆。（注意！这种记忆不会被压缩，但上下文是有限的，所以只能用于记录那些珍贵的核心经验和高优先记忆）")]
    ...
    public override async Task AwakeAsync(AwakeContext context)
    {
        ...
        {
            Description = "提供一系列让你可以回想或记录记忆的工具。",
            Explain = $"""
                       所有记录都会被永久保存，只是会被定期压缩，然后以记忆存档的形式留在上下文中，所以利用多次{nameof(Recall)}你甚至查到最原始的聊天记录。此外你也可以尝试使用{nameof(Search)}，它会尝试在外部的原始记录中搜索记忆。当遇到记不起来的事情时，一定要积极使用上述工具。
                       ...
                       【记忆压缩要点】
                       上下文是有限的，所以当内容过多时就会进行压缩。记忆非常重要，但压缩意味着记忆将被损耗，所以为了确保记忆的质量，必须遵循以下要点。
                       ## 记忆优先级
                       不同的事情值得记下的程度是不一样的。因此压缩记忆时，各经历的占比不是均衡的。要注意，不是对话越多、时间越新的经历就值得记住，真正让人难忘的是珍贵的，那些难得的点点滴滴。
        ...
        : TextCompressor
    {
        public override async Task<string?> Compress(string text)
        {
            ...
            history.AddMessage(AuthorRole.User,
                $"""
                 【来自系统通知】
                 由于上下文过多，现在需要对部分内容进行压缩处理。下面会给予你一段过往时间的聊天记录或记忆存档。这些内容是即将移出上下文的内容，所以需要你用第一人称简述一下发生的事情，方便日后回忆。
                 ...
                 （接下来请直接开始讲述上述压缩内容中涉及的事件信息，不要混入与该事件描述无关的内容，不要在开头回复‘好的’、‘我来总结’这类语句）
                 """);
