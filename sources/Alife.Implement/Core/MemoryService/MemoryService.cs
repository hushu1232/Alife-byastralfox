    ...
    : InteractivePlugin<MemoryService>, IConfigurable<MemoryConfig>
{
    ...
    [XmlFunction]
    [Description("创建一个永久（最高层）记忆。（注意！这种记忆不会被压缩，但上下文是有限的，所以不能记录大批量内容！建议仅用于存储补充性的珍贵记忆，而对于大批量内容可以先记录到文件，然后记录文件的索引即可）")]
    ...
    public override async Task AwakeAsync(AwakeContext context)
    {
        ...
        {
            Description = $"提供一系列让你可以回忆过往的工具，你甚至可以借此查到所有最原始的聊天记录，在遇到记不起来的事情时，你要积极去使用 {nameof(Recall)},{nameof(Search)} 这类工具。",
            Explain = $"""
