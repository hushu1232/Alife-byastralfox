param(
    [int]$Port = 9880,
    [string]$ConfigPath = "D:\Alife\Runtime\TTS\configs\tts_infer_xiayu.yaml",
...
$StdoutLogPath = Join-Path $LogDirectory "xiayu-$Port.out.log"
$StderrLogPath = Join-Path $LogDirectory "xiayu-$Port.err.log"
...
if (-not (Test-Path -LiteralPath $ConfigPath)) {
    throw "XiaYu GPT-SoVITS config not found: $ConfigPath"
}

if (Test-GptSoVitsEndpoint -ProbePort $Port) {
    Write-Host "XiaYu GPT-SoVITS already responds on http://127.0.0.1:$Port."
    return
}

New-Item -ItemType Directory -Force -Path $LogDirectory | Out-Null
Set-Content -LiteralPath $StdoutLogPath -Value "[$(Get-Date -Format o)] starting XiaYu GPT-SoVITS on 127.0.0.1:$Port"
...
Write-Host "Started XiaYu GPT-SoVITS launcher on 127.0.0.1:$Port (launcher PID $($process.Id))."
...
while ([DateTimeOffset]::Now -lt $deadline) {
    ...
    if (Test-GptSoVitsEndpoint -ProbePort $Port) {
        Write-Host "XiaYu GPT-SoVITS is ready on http://127.0.0.1:$Port."
        return
    }
}

Write-Host "XiaYu GPT-SoVITS launch submitted; readiness not confirmed within $StartupProbeSeconds seconds."
