...
| Existing local file upload to group | Natural upload command | Owner only | Prefer XiaYu unless explicitly configured | Group | High | `QChatIntentClassifier.ClassifyFileUpload` must be confirmed + file safety check. Metadata-only candidates must not upload. | Reply/outbox if long | `QChatIntentClassifier.cs`, `QChatFileSafetyService.cs`, `QChatService.cs` | `QChatIntentClassifierTests.cs`, `QChatFileSafetyServiceTests.cs`, `QChatServiceAdapterTests.cs` |
...
- Should existing local file upload remain XiaYu-preferred or become XiaYu-only?
